...
//login user and checking their email and password matching
exports.loginUser = catchAsyncErrors(async (req, res, next) => {
  const { email, password } = req.body;
  //check if user has given email and password both or not
  if (!email || !password) {
    return next(
      new ErrorHandler("Please provide email and password both ", 400)
    );
  }
...
exports.updateProfile = catchAsyncErrors(async (req, res, next) => {
  ...
  //here is the code for cloudinary
  if (req.body.avatar !== "") {
    const user = await User.findById(req.user.id);
